...
export default function ListMenusAdmin() {
  ...
  const handleCheckboxChange = (dia, state, setState) => {
    const currentDays = state.DiasDisponibles
      ? state.DiasDisponibles.split(", ").filter((d) => d !== "")
      : [];
    let updatedDays;
  ...
  const handleCreate = async () => {
    const dataParaEnviar = {
      ...menuNuevo,
      Disponibilidad: menuNuevo.DiasDisponibles.split(", ")
        .filter((d) => d !== "")
        .map((dia) => ({ DiaSemana: dia })),
    };
    await MenuService.create(dataParaEnviar);
    setOpenNuevo(false);
    setMenuNuevo({
      Nombre: "",
      HoraInicio: "",
  ...
  const handleSave = async () => {
    const dataParaEnviar = {
      ...menuEditar,
      Disponibilidad: menuEditar.DiasDisponibles
        ? menuEditar.DiasDisponibles.split(", ")
            .filter((d) => d !== "")
            .map((dia) => ({
              DiaSemana: dia,
            }))
    ...
    try {
      await MenuService.update(menuEditar.IdMenu, dataParaEnviar);
      setOpen(false);
      cargarMenus();
    } catch (error) {
              ...
              >
                ...
                >
                  <Button
                    variant="contained"
                    size="small"
                    sx={{ bgcolor: disponible ? "#FF8C00" : "#ccc" }}
                    disabled={!disponible}
      ...
      >
        ...
        <DialogContent>
          <TextField
            ...
            margin="dense"
            label="Nombre"
            value={menuNuevo.Nombre}
            onChange={(e) =>
              setMenuNuevo({ ...menuNuevo, Nombre: e.target.value })
            }
          />
          <TextField
            fullWidth
            ...
            value={menuNuevo.HoraInicio}
            onChange={(e) =>
              setMenuNuevo({ ...menuNuevo, HoraInicio: e.target.value })
            }
          />
          <TextField
            fullWidth
            ...
            value={menuNuevo.HoraFin}
            onChange={(e) =>
              setMenuNuevo({ ...menuNuevo, HoraFin: e.target.value })
            }
          />
          ...
          {diasSemana.map((dia) => (
            <FormControlLabel
              ...
              control={
                <Checkbox
                  checked={menuNuevo.DiasDisponibles.includes(dia)}
                  onChange={() =>
                    handleCheckboxChange(dia, menuNuevo, setMenuNuevo)
                  }
                />
              }
              label={dia}
      ...
      >
        ...
        <DialogContent>
          <TextField
            ...
            margin="dense"
            label="Nombre"
            value={menuEditar?.Nombre || ""}
            onChange={(e) =>
              setMenuEditar({ ...menuEditar, Nombre: e.target.value })
            }
          />
          <TextField
            fullWidth
            margin="dense"
            label="Hora Inicio"
            value={menuEditar?.HoraInicio || ""}
            onChange={(e) =>
              setMenuEditar({ ...menuEditar, HoraInicio: e.target.value })
            }
          />
          <TextField
            fullWidth
            margin="dense"
            label="Hora Fin"
            value={menuEditar?.HoraFin || ""}
            onChange={(e) =>
              setMenuEditar({ ...menuEditar, HoraFin: e.target.value })
            }
          />
          <Typography sx={{ mt: 2 }}>Días Disponibles:</Typography>
          {diasSemana.map((dia) => (
            <FormControlLabel
              ...
              control={
                <Checkbox
                  checked={menuEditar?.DiasDisponibles?.includes(dia) || false}
                  onChange={() =>
                    handleCheckboxChange(dia, menuEditar, setMenuEditar)
                  }
                />
              }
              label={dia}
